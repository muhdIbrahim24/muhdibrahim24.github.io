(function () {
  ...
  function projectFor(id) {
    ...
    return { id: id, title: '[Project title]', focus: 'Project details pending', summary: 'Verified project information will be added here.', media: [] };
  }
  ...
  function setupNavigation() {
    ...
    function setOpen(open, restoreFocus) {
      ...
      document.body.classList.toggle('nav-open', open && window.innerWidth <= 920);
    ...
    window.addEventListener('resize', function () { if (window.innerWidth > 920) setOpen(false, false); }, { passive: true });
  }
  ...
  function revealTitle(container) {
    all('.title-word', container).forEach(function (word, index) {
      animate(word, [{ opacity: 0, transform: 'translateY(105%)' }, { opacity: 1, transform: 'translateY(0)' }],
  ...
  function revealScene(scene) {
    ...
    var reverse = Boolean(one('.scene-layout-reverse', scene));
    animate(copy, [{ opacity: 0, transform: 'translateY(18px)' }, { opacity: 1, transform: 'translateY(0)' }],
      { duration: 480, delay: 100, easing: 'cubic-bezier(.16,1,.3,1)', fill: 'both' });
    animate(imageButton, [{ opacity: 0, clipPath: reverse ? 'inset(0 0 0 100%)' : 'inset(0 100% 0 0)' }, { opacity: 1, clipPath: 'inset(0)' }],
      { duration: 680, easing: 'cubic-bezier(.16,1,.3,1)', fill: 'both' });
  }

  function revealList(list) {
    all(':scope > [data-motion-item]', list).forEach(function (item, index) {
      animate(item, [{ opacity: 0, transform: 'translateY(14px)' }, { opacity: 1, transform: 'translateY(0)' }],
        { duration: 420, delay: Math.min(index, 8) * 50, easing: 'cubic-bezier(.16,1,.3,1)', fill: 'both' });
    });
  }

  function revealStandalone(item) {
    animate(item, [{ opacity: 0, transform: 'translateY(14px)' }, { opacity: 1, transform: 'translateY(0)' }],
      { duration: 440, easing: 'cubic-bezier(.16,1,.3,1)', fill: 'both' });
  }

  function revealTarget(target) {
    if (!target || target.dataset.motionDone === 'true') return;
    target.dataset.motionDone = 'true';
    if (target.hasAttribute('data-motion-title')) revealTitle(target);
    else if (target.hasAttribute('data-motion-scene')) revealScene(target);
    else if (target.hasAttribute('data-motion-list')) revealList(target);
    else revealStandalone(target);
  }
  ...
  function finishAllMotion() {
    ...
    all('[data-motion-title], [data-motion-scene], [data-motion-list], [data-motion-item]').forEach(function (target) { target.dataset.motionDone = 'true'; });
  }

  function setupMotion() {
    wrapHeadings();
    if (!reducedMotion()) {
      all('.hero-word').forEach(function (word, index) {
        animate(word, [{ opacity: 0, transform: 'translateY(105%)' }, { opacity: 1, transform: 'translateY(0)' }],
          { duration: 720, delay: 70 + index * 130, easing: 'cubic-bezier(.16,1,.3,1)', fill: 'both' });
      });
      animate(one('.hero-rule'), [{ transform: 'scaleX(0)' }, { transform: 'scaleX(1)' }],
        { duration: 760, delay: 210, easing: 'cubic-bezier(.16,1,.3,1)', fill: 'both' });
      animate(one('.hero-summary'), [{ opacity: 0, transform: 'translateY(16px)' }, { opacity: 1, transform: 'translateY(0)' }],
        { duration: 520, delay: 260, easing: 'cubic-bezier(.16,1,.3,1)', fill: 'both' });
    }
    var targets = all('[data-motion-title], [data-motion-scene], [data-motion-list]');
    all('[data-motion-item]').forEach(function (item) { if (!item.closest('[data-motion-list]')) targets.push(item); });
    if (reducedMotion() || !('IntersectionObserver' in window)) { targets.forEach(revealTarget); return; }
    state.motionObserver = new IntersectionObserver(function (entries, observer) {
  ...
  function setupProjects() {
    ...
    function updateControls(project) {
      ...
      if (counter) counter.textContent = 'Image ' + position + ' of ' + length;
      if (lightboxCounter) lightboxCounter.textContent = 'Image ' + position + ' of ' + length;
    ...
    function mediaLabel(media, index, length) {
      ...
      return 'Show image ' + (index + 1) + ' of ' + length;
    }
    ...
    function showMedia(project, index, announceChange) {
      if (!project.media.length) {
        ...
        if (futurePanel) futurePanel.textContent = 'Project media will be added here.';
        if (caption) caption.textContent = 'No project media has been supplied.';
      ...
      loader.onload = function () {
        ...
        if (announceChange) announce('Image ' + (state.mediaIndex + 1) + ' of ' + project.media.length + ' loaded.');
      };
    ...
    function openProject(trigger) {
      ...
      if (PROJECT_IDS.indexOf(id) === -1) return;
      var project = projectFor(id);
      state.projectId = id;
      state.mediaIndex = 0;
      state.opener = trigger;
      if (caseNumber) caseNumber.textContent = 'Project ' + id;
      caseTitle.textContent = project.title;
      if (caseSummary) caseSummary.textContent = project.summary + (project.focus ? ' ' + project.focus + '.' : '');
    ...
    function openLightbox() {
      ...
      if (lightboxTitle) lightboxTitle.textContent = projectFor(state.projectId).title;
    ...
    document.addEventListener('click', function (event) {
      var trigger = event.target.closest('.project-trigger[data-project]');
      if (trigger) { event.preventDefault(); openProject(trigger); }
    });
    thumbnails.addEventListener('click', function (event) {
      var button = event.target.closest('.media-thumbnail[data-media-index]');
      if (button && state.caseOpen) showMedia(projectFor(state.projectId), Number(button.dataset.mediaIndex), true);
    });
    if (previous) previous.addEventListener('click', function () { showMedia(projectFor(state.projectId), state.mediaIndex - 1, true); });
    if (next) next.addEventListener('click', function () { showMedia(projectFor(state.projectId), state.mediaIndex + 1, true); });
    ...
    caseDialog.addEventListener('keydown', function (event) {
      if (state.lightboxOpen) return;
      if (event.key === 'ArrowLeft' && previous && !previous.disabled) { event.preventDefault(); showMedia(projectFor(state.projectId), state.mediaIndex - 1, true); }
      if (event.key === 'ArrowRight' && next && !next.disabled) { event.preventDefault(); showMedia(projectFor(state.projectId), state.mediaIndex + 1, true); }
    });
  ...
  function setupMotionPreference() {
    ...
    function changed(event) { if (event.matches) finishAllMotion(); hydrateFeatureImages(); }
