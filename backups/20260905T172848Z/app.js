...
(function () {
  ...
  function loadVideos(record) {
    ...
    list.forEach(function (item) {
      ...
      if (item.title) {
        ...
        title.className = 'sheet-video-title';
  ...
  /* ------------------------------------------- activity sheet + viewer
     ...
     record across the top, an images-only contact rail on the left, and a
     mixed gallery of photographs and clips in the middle. Everything above
     this point — the case sheet and its lightbox — is untouched. */
  ...
  function planTile(item, index, remaining, cols, solo) {
  ...
  function mediaTile(item, index, shape) {
    ...
    if (item.kind === 'video') {
      ...
      badge.textContent = (item.note || '').split(' · ')[0] || 'Clip';
  ...
  function paintGallery(media) {
    ...
    items.forEach(function (item, i) {
      var shape = planTile(item, i, remaining, cols, solo);
  ...
  function paintRail(media) {
    ...
    items.forEach(function (item, i) {
      aRailItems.appendChild(mediaTile(item, offset + i, { span: 1, ratio: '3 / 4' }));
    });
  }
  ...
  function showMedia(index) {
    ...
    if (mboxCite) {
      var bits = [item.kind === 'video' ? 'Clip' : (item.fit === 'contain' ? 'Document' : 'Photograph')];
      if (item.note) bits.push(item.note);
      mboxCite.textContent = bits.join(' · ');
    }
