(function () {
  ...
  function setupNavigation() {
    ...
    function setOpen(open, restoreFocus) {
      ...
      document.body.classList.toggle('nav-open', open && window.innerWidth <= 1040);
    ...
    window.addEventListener('resize', function () { if (window.innerWidth > 1040) setOpen(false, false); }, { passive: true });
  }
  ...
  function setupMotionBidirectional() {
    ...
    if (!state.motionScrollBound) {
      window.addEventListener('scroll', function () {
        ...
        state.motionRaf = window.requestAnimationFrame(function () {
          var current = window.scrollY || 0;
          if (current !== state.lastScrollY) state.scrollDirection = current > state.lastScrollY ? 'down' : 'up';
          state.lastScrollY = current; state.motionRaf = 0;
        });
      }, { passive:true });
      state.motionScrollBound = true;
    }
    state.motionObserver = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        ...
        else if ((rect.bottom < 0 || rect.top > window.innerHeight) && current !== 'idle') arm(entry.target, state.scrollDirection);
      });
  ...
  function setupProjects() {
    ...
    var caseBrief = one('#case-brief');
    var caseContribution = one('#case-contribution');
    var caseMethod = one('#case-method');
    var caseResult = one('#case-result');
    var caseLimitations = one('#case-limitations');
    ...
    function openProject(trigger) {
      ...
      if (caseFocus) caseFocus.textContent = project.focus || 'Engineering project';
      if (caseSummary) caseSummary.textContent = project.summary || '';
      var details = project.details || {};
      if (caseBrief) caseBrief.textContent = details.brief || 'No verified brief is available.';
      if (caseContribution) caseContribution.textContent = details.contribution || 'No verified contribution is available.';
      if (caseMethod) caseMethod.textContent = details.method || 'No verified method is available.';
      if (caseResult) caseResult.textContent = details.result || 'No verified result is available.';
      if (caseLimitations) caseLimitations.textContent = details.limitations || 'No verified limitation note is available.';
