...
Research → 04 Experience → 05 Activities → 06 Contact. Nav collapses to a
disclosure at 1000px; the wordmark's role tag drops at 620px.

There is no standalone evidence section. All 36 report figures live inside the
case sheet, which opens as a two-pane spread: an ink figure viewer on the left
that steps through one project's plates at a time (arrow buttons, ← → keys,
position line, progress rail, crossfade between plates), and the written record
on the right. A record with no report figures shows its single labelled stock
plate instead of an empty carousel. The two panes stack below 1080px.
